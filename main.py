...
def kfold_split_data(dataset, trian_idx, val_idx, batch_size):
    X_train = torch.as_tensor(dataset['X'].loc[train_idx].values).float()
    y_train = torch.as_tensor(dataset['y'][train_idx].reshape(-1, 1)).float()
    X_val = torch.as_tensor(dataset['X'].loc[val_idx].values).float()
    y_val = torch.as_tensor(dataset['y'][val_idx].reshape(-1, 1)).float()
...
if __name__ == '__main__':
    ...
    if config.mode == 'train':
        ...
        min_val_loss = np.inf
        for fold, (train_idx, val_idx) in enumerate(kfold.split(dataset['X'], dataset['y'])):
            train_dl, val_dl = kfold_split_data(dataset, train_idx, val_idx, config.batch_size)
            ...
            for epoch in range(config.epochs):
                ...
                print('Accuracy for fold %d: %d %%' % (fold, 100.0 * float(correct) / float(total)))
